...
sub dbconnect {
  ...
  my ($dbhost,$dbuser,$dbpwd);
  if ($dbname eq 'formcont') {
     $dbuser  = 'formcont';
     $dbpwd   = '59';
     $dbhost  = 'localhost';
  } else {
     $errmsg = "database pas reconnue";
     return;
  }
...
sub dbquery {
  ...
  my $dbname = 'formcont';
  $dbh{$dbname} = dbconnect($dbname) unless $dbh{$dbname};
  my $sth = $dbh{$dbname}->query ($sql);
  unless ($sth) {
    warn "formcont :: dbquery: ** ERR ** $sql\n";  
    $sth = $dbh{$dbname}->query ($sql);
  }
  return $sth;
}
#--------
...
<div class="logobox"><a href="http://www.formation-continue-unil-epfl.ch/"><img src="http://www.formation-continue-unil-epfl.ch/images/logo.png" alt="Formation continue UNIL/EPFL" border="0"></a></div>
